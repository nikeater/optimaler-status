...
| 2.4.4 Link purpose (in context) | reviewed | Every link says where it goes in words - "Antrag stellen (Phase 1)", "Zum Postfach (nur Ansicht)", "Zu den Eval-Metriken". There is no "hier" and no "mehr". The call-to-action links are LINKS styled as buttons, not buttons pretending to navigate: they work with scripting off and appear once each in the tab order. **Part 24 closed the one exception on a citizen page:** the counterparty surface linked the applicant's case with the case id for its link text, which says where it goes only to a reader who takes the label beside it as part of the link - which is exactly what somebody tabbing from link to link does not do. The link text is now the destination and the id follows it in a `code` element. |
